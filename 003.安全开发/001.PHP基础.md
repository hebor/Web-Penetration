...
2. 获取PHP可执行文件路径
   ...
   ![PHP环境变量-2](../image/Part_3/PHP环境变量-2.png)

3. 配置环境变量，通过`rundll32 sysdm.cpl,EditEnvironmentVariables`打开环境变量配置页面

   ![PHP环境变量-3](../image/Part_3/PHP环境变量-3.png)

PHP环境变量设置完成后可以直接通过CMD或VScode的终端窗口直接执行".php"文件，VScode可以通过`Code Runner`插件更便捷的运行PHP文件，PHP通过readline()、fgets()函数可以获取用户输入的内容，实现简单的交互功能
...
PHP将字符串赋值到变量时，字符串可以是引号内的任何文本，可以使用单引号或双引号定义字符串。但在字符串中引用变量时需要注意，双引号能够解析变量，引用的变量在输出时解析为变量值；单引号不能解析变量，输出时直接以字符串形式输出变量名称。注意双引号和单引号的区别
...
从为空数组逐个定义键值对这个操作来看，数组的下标数字并不代表该数组实际的长度，从`$username[3]`开始定义键值对，只代表该数组中存在`$username[3]`的键值对，无法判断数组中是否存在`$username[1]、$username[0]`等键值对
...
<html lang="en">
        ...
        <style>
            ...
            <?php
                foreach ($contentArray as $key => $value) {
                    //.number0{ font-size: 12px;}
                    echo('.number'.$key.'{font-size: '.(12+(count($contentArray)-$key-1)*5).'px;}');
                }
            ?>
        </style>
    </head>
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="text-area">
            <?php
                if ($key == count($contentArray)-1) {    //取数组最后一位标记
                    echo('<span class="number active number'.$key.'">'.($key+1).'</span>');
                } else {
                    echo('<span class="number number'.$key.'">'.($key+1).'</span>');
                }
            ?>
...
函数是一段可重复使用的代码块，可以将一系列操作封装起来，使代码更加模块化、可维护和可重用，用于节省开发者的开发时间和代码量，提高编程效率。函数又分为内置函数和自定义函数，内置函数指的是语言自带的函数，PHP本身就存在很多内置函数，例如之前学习过的print_r()、count()都是内置函数，通过time()、date('Y-m-d H:i:s')等内置函数可用于获取当前时间

由于内置函数的数量和功能都相当庞大，这个只能在实际项目中边实践边学习，函数章节中主要聊的还是自定义函数
...
// 获取当前的时间
function nowTime(){
    echo time();    //time()函数输出的是时间戳，是整数类型
    var_dump( time() );
	echo date('Y-m-d H:i:s');    //date()函数内部格式并非固定，可自定义，例如date('Ymd His')
}
...
缺省情况下，无论函数的传参是否属于Number类型，在函数内都可以进行算数运算，通过declare()函数启用严格模式的情况下，在定义函数时就可以指定参数的数据类型，传参时就只能使用指定的数据类型，否则报错
...
从上面的函数示例中都直接使用echo()直接输出自定义函数运行结果了，但实际上直接使用echo()的自定义函数场景很少见，大多数情况下会使用return返回函数的运算结果

一般情况下，函数在运行完成后一般都会有一个返回值，这个返回值就是通过return返回的。例如在此前的学习中，除了使用echo()输出运算结果以外，还使用到了print_r()函数，print_r()函数的返回值始终为1

return与echo()的区别在于，只有return才能返回函数的返回值，如果不使用return的话，该函数就没有返回值；并且，在使用return返回函数的返回值时，如果不再手动输出打印计算结果的话，return不会产生任何回显
...
plus(2, 3);    //直接运行函数不会产生任何回显
echo plus(2, 3);    //手动通过echo()输出函数的计算结果
var_dump(plus(2, 3));    //通过var_dump()查看函数的返回值的类型
?>
...
plus(2, 3);    //直接运行函数输出计算结果
var_dump(plus(2, 3));    //通过var_dump()查看函数的返回值的类型为NULL，因为函数没有返回值
?>
```

## 八、变量作用域

学习完函数，就可以回头学习前面提到过的变量作用域了，变量的作用域是脚本中变量可被引用/使用的部分。

**局部 和 全局(global) 作用域**

- 在函数外部定义的变量是全局变量，拥有全局作用域
- 全局变量可以在程序的任何地方调用，但不能在函数内直接调用全局变量
- 需要在函数内调用一个全局变量时，需要使用`global`关键字
...
$user="hebor";    //定义全局变量
function test(){
    echo($user);    //函数内直接调用全局变量时，在IDE内会有语法报错提示，使用命令行执行代码时不会产生任何输出，使用浏览器访问文件时会产生报错提示
	global $user;    //调用全局变量
    echo($user);     //再次打印变量值
	$age="35";       //定义局部变量
	echo($age);      //函数内打印局部变量
...
test();
echo($age);    //全局调用局部变量时在IDE内会有语法报错提示
?>
```
...
```php
#myTest()函数调用完成时会删除变量$a，每次调用myTest()函数时都会重新执行一遍函数内所有的代码，$a会被反复归零
<?php
function myTest() {
    $a=0;    //归零
    echo($a);
...
#通过static保持$a在函数完成后不被删除，那么只要$a被赋值过，$a会携带已有的赋值，在函数下次执行时，$a会以携带的值参与函数的执行
<?php
function myTest() {
    static $a=0;
    echo($a);
...
myTest();    //首次输出$a的值为0，函数内$a自增运算一次
myTest();    //输出$a的值为1，$a再次自增运算
myTest();
?>
```
...
在创建自定义函数时可以为函数添加参数，在调用函数时进行传参，等同于为函数的局部变量进行赋值
...
test("hebor");
var_dump($user);    //即便从全局向函数传参，但仍属于局部变量，无法外部调用
?>
```

### 8.3 unset和isset函数

unset()可以删除其所在作用域内的变量，isset()可以判断作用域内变量是否存在
...
#unset()函数可以局限在不同的作用域生效
<?php
$user="hebor";
function test() {
    global $user;       //调用全局变量
    var_dump($user);    
    unset($user);       //在函数中删除全局变量
    var_dump($user);    //函数中无法再调用全局变量
}
test();
var_dump(isset($user));    //全局作用域下正常调用全局变量
?>
...
function test() {
    ...
    var_dump($user);
    unset($user);
    var_dump(isset($user));    //删除局部变量后，函数内部也无法再调用
}
test();
?>
```
...
相比较全局变量，超级全局变量的作用域更大，全局变量的作用域仅限于声明该全局变量的文件中，超级全局变量的作用域在整个项目中

- $GLOBALS：用于在PHP脚本中的任意位置访问全局变量
- $_SERVER：用于保存关于报头、路径、脚本位置等相关信息
- $_REQUEST：用于收集HTML表单提交的数据
- $_POST：广泛用于收集提交method="post"的HTML表单数据
- $_GET：收集URL中发送的数据，也可用于收集提交method="get"的HTML表单数据
- $_FILES：文件上传且处理包含通过HTTP POST方法上传给当前脚本的文件内容
- $_ENV：是一个包含服务端环境变量的数组
- $_COOKIE：是一个关联数组，包含通过cookie传递给当前脚本的内容
- $_SESSION：是一个关联数组，包含当前脚本中所有的session内容

### 9.1 $GLOBALS 

`$GLOBALS`是一个包含了全部变量的全局组合数组，重点在于`$GLOBALS`是一个多维数组，在这个数组中，变量的名字就是数组的键、变量的值就是数组的值
...
通过浏览器访问该数组的值，可以看到`$GLOBALS`中默认就保存了其他超级全局变量的空数组
...
基于此前的自定义函数的示例来观察`$GLOBALS`超级全局变量

```php
<?php
$user="hebor";    //创建全局变量
$age="27";
print_r($GLOBALS);    //查看$GLOBALS超级全局变量的值是否产生变化
function test() {
    global $user, $age;
    $age="38";    //在函数内修改全局变量的值
    $GLOBALS["user"]="nannan";    //通过$GLOBALS数组修改全局变量的值
}
test();
echo($user.PHP_EOL);
echo($age.PHP_EOL);
...
虽然超级全局变量的作用域在整个项目中，即可跨文件调用，但仅限于超级全局变量本身的值可以跨文件调用，例如，上述示例文件中`$GLOBALS`超级全局变量本身可以跨文件调用，但创建的全局变量`$user`、`$age`的值不可以跨文件调用

###  9.2 $_SERVER

- `$_SERVER` 是一个包含了诸如头信息（header）、路径（path）、以及脚本位置（script locations）等等信息的数组。
- 这个数组中的项目由Web服务器创建。
- 各个服务器之间不完全相同，因此不能保证每个Web服务器都能提供全部参数。不同的Web服务器可能会忽略一些、或者提供一些参数。
...
`$_SERVER`是一个数组，直接输出该数组的值即可看到`$_SERVER`数组包含哪些参数，常见的参数解析如下

| 参数                        | 描述                                                         |
| :-------------------------- | :----------------------------------------------------------- |
| $_SERVER['PHP_SELF']        | 当前执行脚本的文件名，与`document root`有关，以网站根目录为根获取执行的程序文件。例如，URL是`http://example.com/test.php/foo.bar`时，通过`$_SERVER['PHP_SELF']`将得到`/test.php/foo.bar` |
| $_SERVER['SERVER_ADDR']     | 获取脚本所在的服务器的IP地址                                 |
| $_SERVER['SERVER_NAME']     | 获取脚本所在的服务器的主机名。若脚本运行在虚拟机中，此参数会获取虚拟机的主机名 |
| $_SERVER['SERVER_PROTOCOL'] | 请求页面时通信协议的名称和版本                               |
| $_SERVER['REQUEST_METHOD']  | 客户端访问Web页面使用的HTTP请求方法。例如，"GET", "HEAD"，"POST"，"PUT" |
| $_SERVER['REQUEST_TIME']    | 请求开始时的时间戳，从 PHP 5.1.0 起可用                      |
| $_SERVER['QUERY_STRING']    | 获取客户端发起请求时携带的参数                               |
| $_SERVER['HTTP_HOST']       | 获取当前请求头中的Host字段的值，这个字段的值可能是域名或IP   |
| $_SERVER['HTTP_REFERER']    | 获取请求头的REFERER值，由user agent设置决定。并不是所有的用户代理都会设置该参数，且该参数可被篡改 |
| $_SERVER['HTTPS']           | 如果脚本是通过HTTPS协议被访问，则被设为一个非空的值          |
| $_SERVER['REMOTE_ADDR']     | 获取浏览当前页面的客户端IP                                   |
| $_SERVER['REMOTE_HOST']     | 获取浏览当前页面的客户端主机名。DNS反向解析不依赖于客户端的REMOTE_ADDR |
| $_SERVER['REMOTE_PORT']     | 客户端连接到Web服务器所使用的端口号                          |
| $_SERVER['SCRIPT_FILENAME'] | 当前执行脚本的绝对路径                                       |
| $_SERVER['SERVER_PORT']     | Web 服务器使用的端口。默认值为 "80"。如果使用 SSL 安全连接，则这个值为用户设置的 HTTP 端口。 |
| $_SERVER['SCRIPT_NAME']     | 包含当前脚本的路径。这在页面需要指向自己时非常有用。__FILE__ 常量包含当前脚本的完整路径和文件名 |
| $_SERVER['REQUEST_URI']     | 包含了完整的请求资源的路径和客户端携带的参数。例如 `/index.php?user=hebor` |
| $_SERVER['REQUEST_SCHEME']  | 判断客户端请求是http或https                                  |
...
$numString = readline("Please input your number: ");
$num = (int) $numString;
print_r(getFactors($num));
?>
```
...
- `+`号运算符（合并）
  ...
  数组运算符的`+`号运算是将两个数组的键值对合并到一起，但需要注意的是，如果合并的多个数组中存在相同的键，那么在数组合并的过程中只会保留第一个数组中的键值对，而忽略后面数组中相同键名的元素。也就是说`+`号运算会保留最老的键值对

  如果想要合并两个数组并取后面数组的键值对，也就是取最新的键值对，可以使用array_merge()函数
  ...
  $b=[
      "m"=>"测试2",    //在没有相同键名的元素时，+运算就只是单纯的合并多个数组
      "o"=>"测试o",
      "p"=>"测试p",
      "q"=>[
          "cs"=>"二维测试"
  ...
  print_r($a+$b);    //$a在前，则保留$a数组中的键值对
  print_r($b+$a);    //$b在前，则保留$b数组中的键值对
  $c=[
      "m"=>"测试3",
      "cc"=>123
  ];
  print_r($a+$b+$c);
  $result=array_merge($a, $b, $c);    //取最新的键值对
  print_r($result);
  ?>
  ```

- `==`、`===`
  与算数运算符一样，`==`用于判断两个数组的数值是否相同，`===`用于判断两个数组有相同的键/值对，且顺序相同、类型相同

```php
<?php
$a=[1, 2, 3];
$b=[1, 2, "3"];
var_dump($a==$b);    //两个数组的键值对相同，结果为True
var_dump($a===$b);   //数组类型不一致，结果为False
$a=[1, 2, 3];
$b=[1, 3, 2];
var_dump($a==$b);    //两个数组的键值对不同，结果为False。$a[1]=2,$b[1]=3
$a=[
    "a"=>123,
    "b"=>456
];
...
var_dump($a==$b);    //两个数组的键值对相同，结果为True
var_dump($a===$b);   //数组顺序不同，结果为False
?>
```

- `!=` 和 `<>`、`!==`，与算数运算符类似，`!=`和`<>`都表示不等于，两者效果相同，`!==`与`===`的条件是一样的，只不过`!==`是取反

### 10.2 三元运算符

三元运算符都的计算逻辑其实非常类似于`if...else...`，只不过三元运算符将`if...else...`用一行代码表现出来

```php
<?php
$name=isset($username)?$username:"测试";    //条件判断为True时，返回?后面的值，否则返回:后面的值
var_dump($name);
//用if...else...表现出来如下
if (isset($username)) {
    $name=$username;
...
空合并运算符在PHP 7之前不支持，空合并运算符用于简化处理可能为null的变量或数组元素的情况。它的作用是判断一个变量是否未定义或者为null，如果未定义或为null，则返回指定的默认值；否则返回该变量的值

```php
<?php
$name=$username??"hebor";    //判断$username变量是否已存在，不存在则输出默认值
echo($name);
?>
```

空合并运算符与三元运算符的主要区别在于，如果仅需要检测变量是否存在，优先使用空合并运算符，更安全简洁，需要复杂条件判断时再使用三元运算符

### 10.4 组合比较符

组合比较符在PHP 7之前不支持，可用于比较整型、浮点型、字符串，组合比较符的返回值只有3个：`1`、`0`、`-1`，以`$c = $a <=> $b;`为例：
...
var_dump($a<=>$b);
$b=1.5;
var_dump($a<=>$b);    //整数与浮点数比较
$b="2";
var_dump($a<=>$b);    //整数与字符串数值比较
$a="A";
$b="a";
var_dump($a<=>$b);    //字符串之间的比较遵循ASCII码的数值解析
$a="abc";
$b="abd";
var_dump($a<=>$b);    //字符串前两个值相同，比较最后一个字符串的ASCII码表值
$a="abcff";
var_dump($a<=>$b);    //从字符串的第三个字符比较出结果后，不会再继续比较后面是否还存在其他的字符
?>
```
...
虽然PHP是弱类型语言，但也需要明白变量类型及它们的意义，因为我们经常需要对PHP变量进行比较，包含松散和严格比较。
...
- isset()：可以用于判断变量是否存在，不存在则返回NULL

- is_null()：只能用于判断变量是否为NULL

- empty()：可以用于判断几种特殊的值，但无法具体判断出到底是什么值

```php
<?php
var_dump(!isset($name));
var_dump(is_null($name));    //未定义的变量的返回值是NULL，但直接用is_null()函数判断未定义的变量，可能会产生报错
var_dump(empty($name));      //empty()函数对于未定义的变量会返回True
$name=NULL;
var_dump(empty($name));
?>
```

empty()函数一般不适合用于判断变量是否已被定义，因为除了对于未定义的变量会返回True以外，empty()函数在其他多个特别场景下也会返回True
...
如果需要详细判断变量是否为空字符串、整数0等，此时需要用到全等于进行判断
...
echo(strlen($strZh).PHP_EOL);    //一个中文字符占3个字节
echo(strlen("abc").PHP_EOL);
echo(strpos($strZh, "猫叔").PHP_EOL);    //strpos函数用在判断一个字符串中是否存在某个特定字符比较多，用于寻找字符位置的场景较少
if (strpos($strEn, "H") === False) {    //判断时最好使用全等于，因为“H”字符下标值为0，==会判断0等同于False，返回“不含有”
    echo("不含有".PHP_EOL);
} else {
    var_dump(strpos($strEn, "H"));
}
print_r(explode(" ", $strEn));     //以空格为特征，将字符串打散为数组
$arr=explode(" ", $strEn);
print_r(implode(" -- ", $arr).PHP_EOL);    //通过指定字符，将数组拼接为字符串
print_r(strtoupper($strEn).PHP_EOL);
print_r(strtolower($strEn).PHP_EOL);
print_r(ucfirst("hebor").PHP_EOL);
print_r(lcfirst("HEBOR").PHP_EOL);
print_r(ucwords($strEn).PHP_EOL);
print_r(str_replace("猫叔", "hebor", $strZh).PHP_EOL);
print_r(strrev($strEn).PHP_EOL);
print_r(substr($strEn, 2, 3).PHP_EOL);    //从下标索引为2开始，向后截取3个字符，默认情况下是截取全部字符
print_r(mb_substr($strZh, 2, 3).PHP_EOL);    //截取中文字符
?>
```
...
- stripos() 同上，但不区分大小写
- strrpos() 在字符串内查找一个字符或一段指定的文本，返回最后一次出现的位置或false
- strripos() 同上，但不区分大小写
...
- trim()：去除字符串两端的空格
- substr() 截取字符串的一部分
- mb_substr() 截取中文字符串的一部分，使用此函数需要安装扩展mbstring
...
```php
<?php
function uncwords ($contact)
{
    $wordarr = explode(" ", $contact);
    $lowwordarr = [];
    foreach ($wordarr as $key => $value) {
        if (strpos($value, ",")!==False) {    //此判断用于处理语句中的逗号，默认情况下逗号相连的单词之间不会直接进行处理
            $arr2=[];
            $arr1=explode(",", $value);
        ...
        $lowwordarr[]=lcfirst($value);
    }
    return implode(" ", $lowwordarr);
}
$enter=readline("Please input your contact: ");
print_r(uncwords($enter));
?>
```
...
4. array_unshift($array, $newElement)：将一个或多个元素添加到数组的开头
   ...
   $arr[]="5";    //向数组尾部追加元素
   var_dump($arr);
   array_push($arr, "6", "7");    //array_push可以一次向数组尾部追加多个元素
   var_dump($arr);
   array_unshift($arr, "1", "2");    //array_unshift一次向数组头部追加多个元素
   var_dump($arr);
   
   $arr2=array("a"=>1, "b"=>2);
   array_unshift($arr2, ["c"=>6]);    //array_unshift不能将一个键值对直接插入关联数组
   var_dump($arr2);
   
   $arr2=array("a"=>1, "b"=>2);
   $arr2=array_merge(["c"=>6], $arr2);    //array_merge合并数组
   ```
...
6. array_shift()：删除并返回数组中的第一个元素。 
   ...
   print_r(array_pop($arr2).PHP_EOL);    //array_pop直接变更数组内容，删除并返回数组的尾部元素，但只返回元素值
   print_r($arr2);
   print_r(array_shift($arr2).PHP_EOL);
   print_r($arr2);
   ```
...
7. array_slice()：从数组中提取一部分元素，组成新的数组
   ...
   $result=array_slice($arr2, 2);    //从下标2开始取值，默认取全部元素
   print_r($result);
   $result=array_slice($arr2,2,2);    //从下标0开始取值，取数组中2个元素
   print_r($result);
   $result=array_slice($arr2, 2, -1);    //从下标0开始取值，取到数组倒数第一个元素为止，不包括最后一个元素
   print_r($result);
   $result=array_slice($arr, 1, 2, false);    //array_slice的第4个参数用于决定新数组的下标值是否归零重新开始计算，第4参数只针对数值数组有效，针对关联数组无效
   print_r($result);
   ```
...
9. array_reverse()：反转数组中的元素的顺序
   ...
   $arr=array("a", "b", "c", "d");
   $arr2=array("a"=>1, "b"=>2, "c"=>3, "d"=>4, "e"=>5, "f"=>6);
   print_r(array_reverse($arr2));    //针对关联数组进行反转时，键值对会一起反转
   print_r(array_reverse($arr));     //针对数值数组进行反转时，值会反转、键保持不变，因此数值数组反转后，原本的键值对会产生变化
   ```
...
15. array_unique()：移除数组中的重复值（原数组不变）
    ...
    ```php
    $arr=array("a", "b", "c", "d", "a", "a", "b");
    $arr2=array("a"=>1, "b"=>2, "c"=>3, "d"=>4, "e"=>"1");
    
    var_dump(in_array("a", $arr));
    var_dump(array_key_exists("1", $arr));
    
    var_dump(array_keys($arr));
    var_dump(array_keys($arr, "a"));    //array_keys()函数接收多个参数，第二参数表示匹配数组中值为“a”的键
    var_dump(array_keys($arr2, "1", true));    //array_keys()函数表示是否启用严格模式，默认为false
    var_dump(array_values($arr));
    
    var_dump(array_search("a", $arr));    //array_search()函数只能返回第一个键
    var_dump(array_unique($arr));    //array_unique()函数不会影响到原数组的值
    ```
...
20. array_product() 数组求乘积
    ...
    var_dump(max($arr));    //max()、min()函数可以用于比较数值数组、关联数组、字符串
    var_dump(min($arr2));
    var_dump(max([1, 2, 3, 4, 5]));
    
    var_dump(sort($arr));    //sort()函数直接对原数组进行变更
    var_dump($arr);    //查看数组
    rsort($arr);
    var_dump($arr);
    
    var_dump(array_sum([0, 1, 2, 3, 4]));     //array_sum()函数只能对数值数据进行求和
    var_dump(array_product([1, 2, 3, 4]));    //array_product()函数求乘积，数组里不能有0，否则结果永远为0
    ```
...
- "sorted" ：按照从小到大排序的数组。 
  ...
  function testArray($numbers) {
      $arr=[];
      $arr["元素个数"]=count($numbers);
      $arr["元素总和"]=array_sum($numbers);
      $arr["元素平均值"]=round($arr["元素总和"]/$arr["元素个数"],2);    #round()函数用于保留小数点后2位，用number_format()函数能够实现相同的功能，格式一样
      $arr["数组最大值"]=max($numbers);
      $arr["数组最小值"]=min($numbers);
      sort($numbers);
      $arr["元素升序"]=$numbers;
      return $arr;
  }
  $numbers=array(5, 9, 3, 2, 7, 1, 6);
...
- microtime(true)：返回一个浮点数时间戳（秒和毫秒的总和）
- date(格式，时间戳)：日期格式化

- strtotime(string)：将字符转换为时间戳

  ```php
  <?php
  $time = time();
  echo time();             //输出时间戳（10位）
  echo microtime(true);    //输出时间戳（秒和毫秒的总和），true表示以浮点数形式展现毫秒，false则通过空格拆分毫秒与秒的位置
  
  # JS函数取时间戳一般是13位，如果PHP也要实现输出13位的时间戳，需要多个函数结合算数运算实现
  echo floor(microtime(true)*1000);    //floor()函数向下取整，得到13位世家戳
  echo date('Y-m-d', time());          //date()函数由两个参数组成，输入的日期格式+时间戳，时间戳必须使用秒级，毫秒级会导致date()函数计算错误
  echo date('Y-m-d H:i:s', $time);     //修改日期格式，输出具体的时间
  
  # strtotime()函数的计算结果是时间戳，如果要转换为人类正常识别的时间，仍需要结合date()函数使用
  $time = time();
  echo strtotime("next Monday");       //输出下周一的时间戳
  $result = strtotime("next Monday");
  echo date('y-m-d h:i:s', $result);   //输出格式化后的时间
  
  $baseTime = strtotime("2023-10-01");         //定义一个基准时间
  echo strtotime("+1 day", $baseTime);    //基于基准时间调整时间变化，除了加减day，也可以加减hours等
  echo date('Y-m-d H:i:s', strtotime("+1 day", $baseTime));    //查看时间显示结果
  ?>
  ```

- mktime($hour, $minute, $second, $month, $day, $year)：生成时间戳

- date_create()：来创建一个日期时间对象date_create('2023-11-01')

- date_format()：用于将日期和时间格式化为指定的字符串格式

  ```php
  <?php
  $time = mktime(12, 0, 0, 1, 31, 2026);    //按函数参数格式生成一个指定时间的秒级时间戳
  echo date('Y-m-d H:i:s', $time);
  
  $day = date_create("2026-01-31");    //date_create()函数用于生成一个时间对象，它不能单纯的使用echo()输出
  var_dump($day);
  
  $day = date_create("2026-01-31");    //date_create()函数用于生成一个时间对象，它不能单纯的使用echo()输出。date_create()函数的参数可以仅写日期，或更明细的时间
  var_dump($day);    //var_dump()的输出结果看起来像数组形式，但也只是看起来像
  var_dump($day->getTimezone());       //取时间对象中的“时间区间”部分
  var_dump(date_format($day, "Y年m月d日 H:i:s"));    //date_format()函数的第一个参数必须是对象，第二个参数是想要输出的字符串格式
  echo(date_format($day, "Y-m-d"));    //通过date_format()函数将时间对象转换为字符串格式后可以通过echo()输出
  >?
  ```
...
- date_diff()：计算两个日期之间的差
  ...
  ```php
  <?php
  # date_diff()函数的参数需要两个对象，计算两个时间对象之间的日期之差
  $date1 = date_create('2026-01-30 09:00:00');
  $date2 = date_create('2026-01-31 18:00:00');
  $diff = date_diff($date2, $date1);
  var_dump($diff);    //输出整体对比
  var_dump($diff->format('%a days'));     //仅输出天数的差异
  var_dump($diff->format('%H hours'));    //仅输出小时的差异
  var_dump($diff->format('%Y年 %m月 %a天 %H小时'));
  ?>
  
  -  %Y ：完整年份的差异 
  -  %y ：年份的差异（两位数） 
  -  %m ：月份的差异 
  -  %d ：天数的差异。仅计算天数，例如一个月零一天，仅计算1天的差异
  -  %a ：总共的天数差异。计算总天数，例如一个月零一天，假设一个月是30天，%a会计算31天的差异
  -  %H ：小时的差异 
  -  %h ：小时的差异（12小时制） 
  -  %I ：分钟的差异 
  -  %S ：秒数的差异 
  -  %R ：正负。date_diff()函数默认使用右侧参数-左侧参数（$date2-$date1），当$date2小于$date1时，计算结果是负值，此时%R会在输出的字符串末尾追加“-”号，同样的计算结果为正值时，也会在输出的字符串末尾追加“+”号
  -  %r ：正负。同%R，但计算结果为正值时不在输出的字符串末尾追加“+”号，计算结果为负值时会显示“-”号
  ```
...
- strftime($format, $timestamp)：根据指定的格式，将时间戳格式化为可读的日期和时间字符串，支持本地化的日期和时间格式（**PHP8已废弃**）。
  ...
  <?php
  $timestamp = time();
  $dateString = strftime("%A %Y-%m-%d %H:%M:%S", $timestamp);    //将时间戳转换为易读形式，与date()函数作用相似
  echo $dateString;
  ?>
  ```

- gmdate($format, $timestamp)：根据指定的格式，将GMT时间戳格式化为可读的日期和时间字符串。
  `gmdate()` 和 ]`date()`的区别在于，`gmdate()` 函数使用格林威治标准时间（GMT）作为默认的日期/时间基准，**会忽略服务器的时区设置**，始终返回格林威治标准时间（GMT）的日期和时间；`date()` 函数则**使用本地时间作为基准**，它会根据当前服务器的时区设置来格式化日期和时间。
  
- `date_default_timezone_set($timezone)` ：设置默认的时区。常见的一些时区设置如"UTC"、"localtime"、"Asia/Shanghai"，设置"UTC"等同于将date()函数的时区定位到格林威治标准时间，所以此时gmdate()和date()两个函数输出的时间是相同的，设置"localtime"、"Asia/Shanghai"就是让date()函数使用本地时区，gmdate()函数的时区无法被修改，两个函数输出的值不同

  需要注意，设置默认时区只能在脚本开始时设置，不能再运行时动态设置。
...
- timezone_identifiers_list()：返回所有可用时区标识符的数组。
  ...
  echo date("Y-m-d H:i:s");    //两者时间实际相差8小时
  
  date_default_timezone_set("UTC");    //设置默认时区后，两个时间相同
  echo gmdate("Y-m-d H:i:s".PHP_EOL);
  echo date("Y-m-d H:i:s");
  
  print_r(timezone_identifiers_list());    //查看所有可用的时区
  ```
...
```php
<?php
$dateTime = new DateTime('2026-01-31 19:41:51');    //创建一个时间对象
$dateTime -> modify('+1 day');    //通过modify()方法修改时间间隔
$dateTime -> modify('-1 hour');
echo $dateTime -> format('Y-m-d H:i:s').PHP_EOL;    //通过format()方法格式化输出时间
$dateTime -> setDate(2025, 1, 22);    //setDate()方法用于设置日期，能够保留时间不变
echo $dateTime -> format('Y-m-d H:i:s').PHP_EOL;
$dateTime -> setTime(20, 18, 30);    //与setDate()方法对应，setTime()方法用于设置时间，日期保留不变
echo $dateTime -> format('Y-m-d H:i:s').PHP_EOL;

$timeZone = new DateTimeZone("Asia/Shanghai");    //创建一个时间对象指定时区
$dateTime -> setTimezone($timeZone);    //设置时区，setTimezone()方法的参数必须是对象
$dateTimeZone = $dateTime -> getTimezone();    //获取时区对象的信息
var_dump($dateTimeZone);
var_dump($dateTimeZone -> getName());    //直接获取时区名称
var_dump($dateTimeZone -> getLocation());    //获取本地时区信息

$dateTime = new DateTime('2026-01-31 20:41:32');
$timestamp = $dateTime -> getTimestamp();    //获取秒级时间戳
echo $timestamp.PHP_EOL;

// 另一种修改时间对象的写法，这种写法不如DataTime()的modify()方法简单，一般不常用
$dateTime = new DateTime('2026-01-31 19:41:51');
$interval = new DateInterval('P1D');    //定义一个时间间隔对象，间隔一天
$dateTime -> add($interval);    //通过不同的方法修改时间对象
echo $dateTime -> format('Y-m-d').PHP_EOL;

// 计算时间差
$dateTime1 = new DateTime("2023-12-05");
$dateTime2 = new DateTime("2026-01-31");
$diff = $dateTime1 -> diff($dateTime2);    //使用第一个时间对象dateTime1的diff()方法，与第二个时间对象做减法运算，dateTime2-dateTima1
echo $diff -> format('%R %m %d %h %i %s').PHP_EOL;    //此处diff()方法的格式语法与此前的date_diff()函数的格式语法通用
?>
```

## 十五、特殊的变量写法和常量

特殊写法一般不会用到，但是在特定场景下会使用，需要认识这些写法

### 15.1特殊的变量写法

```php
<?php
$value1 = "test1";       //正常的定义变量、打印变量值
$$value1 = "test2";     //$$value1 等同于 $test1，test1 是变量 $value1 的值
// 双$$特殊写法，首先value1必须已经存在才能用$$特殊写法，双$$写法实际上是将$value1变量的值视作新的变量名，再通过第二个$为新的变量名重新赋值

echo $value1.PHP_EOL;    //打印$value1原本的值
echo $$value1.PHP_EOL;   //这种写法与下面一种写法是一样的结果
echo $test1.PHP_EOL;     //$test1 变量没有定义过，但可以直接输出结果

// 这类嵌套似的特殊的变量命名还能够继续嵌套
$$$value1 = "test3";     //与上面类似，$$$value1 等同于 $test2，test2 是变量 $$value1 的值、也可以说是变量 $test1 的值
echo $$$value1.PHP_EOL;
echo $$test1.PHP_EOL;
echo $test2.PHP_EOL;

// 实际的应用场景一般在for循环中使用，但现在使用这种方式为变量赋值比较少见，更多的是通过数组进行循环赋值，在通过数组下标进行调用
for ($i=0; $i<5; $i++){
    $a = "hebor".$i;    //循环生成不同的变量名 hebor1
    $$a = "test".$i;    //为每个变量名进行赋值 hebor1 = test1
}
echo $hebor1.PHP_EOL;
echo $hebor2.PHP_EOL;
```
...
常量和变量是类似的，特别是学习到PHP的一些其他框架时就会用到一些常量。常量的特征：

1. 常量值被定义后，在脚本的其他任何地方都不能被改变
2. 常量的命名规则和变量类似，严格区分大小写，但无需$符，也不能使用$符
3. 常量默认是全局的，可以在整个运行的脚本的任何地方使用

```php
<?php
// 常量的定义
define("HEBOR0", "Constant learning test1");    //使用define()定义常量
echo HEBOR0.PHP_EOL;

const HEBOR1 = "Constant learning test2";       //使用const定义常量，这种方式更类似变量的定义
echo HEBOR1.PHP_EOL;

// 在一些场景下，常量的命名本身是不固定的，此时常量的定义和调用都需要单独注意
$a = 2;
define("HEBOR".$a, "Constant learning test3");    //定义常量时使用"."号拼接变量命名
//echo "HEBOR".$a    这种方式调用，"HEBOR"会被识别为字符串，输出结果不是常量的值
//echo HEBOR.$a      HEBOR本身不是一个被定义的常量，输出报错
echo constant("HEBOR".$a).PHP_EOL;    //调用命名不固定的常量时通过constant()函数实现
...
#### 15.2.1 const和define的区别

虽然二者都能定义常量，但两者仍存在一些区别：
...
$a = 1;
if($a==1){
    define("HEBOR".$a, "test-1");
    echo constant("HEBOR".$a).PHP_EOL;

//    const HEBOR2 = "test-2";    //IDE中会出现语法错误提示，运行报错
//    echo HEBOR2;
}
?>
```

- const用于类成员变量的定义，可以在类中使用；define可用于全局常量，不可用于类成员变量的定义，不能在类中使用。

```php
<?php
class ceshi {
    const Ceshi = 'HEBOR';
//    define("MS", "test paragraph.");    //define()不可用于类成员常量的定义，IDE会产生语法错误提示，运行会报错
    public function cc(){
        define("MS", "test paragraph.");    //define()可以在方法中定义，且定义的常量全局生效
        echo self::Ceshi;    //类成员定义的常量，在类的cc()方法中，通过self::语法调用类成员常量
    }
}
$c = new ceshi;
$c -> cc();    //调用cc()方法输出const定义的常量
//echo Ceshi;    //在全局环境下调用类成员定义的常量会直接报错
echo MS;       //define()定义的常量全局生效，无须创建对象，直接调用
?>
```
...
- 常量前面没有美元符号（$），而且也不能有
- 常量可以不用理会变量的作用域，常量可以在任何地方定义和访问
- 常量一旦定义就不能重新定义或取消定义，也不能设置为NULL
- 常量的值只能是标量（字符串、整数、浮点数、布尔值），现在也支持数组
...
//print_r(get_defined_constants());    //在一个关联数组中，打印所有常量，自定义常量会追加到数组最末尾
//print_r(get_defined_constants(true));    //按功能作用分类，以二维关联数组的形式打印所有常量
print_r(get_defined_constants(true)["user"]);    //指定指定二维数组，获取数组中的元素，user即自定义的常量
print_r(get_defined_constants(true)["date"]);    //date是时间相关常量
?>
```

### 15.3 魔术常量

常量的基本特征之一：常量值被定义后，在脚本的其他任何地方都不能被改变。而魔术常量之所以被冠以魔术二字，是因为魔术常量虽然不能够被重新赋值，但它的值会随着它在代码中的位置的改变而改变

- `__LINE__`：返回当前行号
- `__FILE__`：返回文件的完整路径和文件名，包含（盘符）根目录
- `__DIR__`：文件所在的目录
- `__FUNCTION__`：该函数被定义时的名字（区分大小写）
- `__CLASS__`：该类被定义时的名字（区分大小写）
- `__NAMESPACE__`：命名空间
- `__METHOD__`：包含了：命名空间、类名、函数名
- `__TRAIT__`：当前使用的trait的名称

```php
<?php
namespace MyProject;    //命名空间的定义一般是在文件开头
//echo "命名空间是：".__NAMESPACE__.PHP_EOL;
//die();    //停止运行程序文件下面的代码
...
echo __DIR__.PHP_EOL;    //注意 __FILE__ 与 __DIR__ 的区别，__FILE__ 返回当前执行的脚本以及脚本所在目录，__DIR__ 仅返回当前执行的脚本所在的目录（不包含最后一个斜杠），不返回脚本文件名称


//function add($a, $b){
//    echo "打印当前函数名".__FUNCTION__;
//    return $a + $b;
//}
//add(2, 3);
...
class testClass {
    function test(){
        echo "类名称是：".__CLASS__.PHP_EOL;
        echo "函数名称是：".__FUNCTION__.PHP_EOL;
...
trait MyTrait {    //trait被用于定义一个能够重复使用的方法
...
包含文件可以在当前文件中引入其他PHP文件、HTML文件、文本文件等，这些被引入的文件一般是包含了公共的方法、公共的页面等。比较常见的公共方法例如将常用的函数写在同一个文件内，引入这个文件；公共页面例如引入header、footer、sider等网页通用部分，PHP引入文件主要使用include和require两个语句

### 16.1 include和require语句

两者在PHP中除了处理错误的方式不同外，其他方面都是相同的

- require：找不到引用的文件时会生成一个致命错误（E_COMPILE_ERROR），在错误发生后脚本会停止执行。
- include：找不到引用的文件时会生成一个警告（E_WARNING），在错误发生后脚本会继续执行。

因此，如果希望被包含的文件是必需的且缺少文件会导致脚本无法正常运行，应使用require（推荐）。 如果希望被包含的文件是可选的，或者即使缺少文件也希望脚本继续执行，可以使用include。
...
# 包含文件：hebor.php
<?php
include "./test.php";    //从执行结果来看，就像是将test.php文件的代码复制到了hebor.php文件中，从上到下逐行zhxing
echo $test;
?>
?>
```
...
<h1>测试，<?php
    /** @var string $siteTitle */    //这个注释只是为了让phpstorm的显示上不报语法错误，没有这个注释代码也能正常运行
    echo $siteTitle;
    ?></h1>
<p>测试include包含文件语句</p>
</body>
...
### 16.2 include_once和require_once语句

两者与include和require语句的作用是对应的，只不过这两者只会调用一次外部文件，防止重复调用
...
echo "测试包含文件";
/** 
 * function ceshi(){    //如果被引用文件中存在函数或其他不可被重复定义的代码，在重复调用时会直接报错
 *   echo "---";
 * }
 */
?>
...
<?php
//include "./test.php";
//include "./test.php";    //include允许外部文件重复调用，并切会输出两次运行结果
include_once "./test.php";
include_once "./test.php";    //include_once语法可以出现多次，但调用外部文件只有一次
/** @var string $test */
echo $test;
?>
```
...
[模板文件](https://maoshu.fun/articles/775.html)从猫叔的知识星球下载，将模板文件解压到一个网站根目录下。模板文件的Web页面中，页面的顶部区域、尾部区域、侧边栏区域三者颜色或格式作为公共部分，通过包含文件实现同样的页面。
...
├── 解压后的模板文件
├── common    //通用文件存放目录
│     ├── footer.php    //Web页面的尾部区域文件
│     ├── header.php    //Web页面的头部区域文件
│     ├── head.php      //HTML的head标签代码文件
```

#### 16.3.2 分离通用文件——尾部

1. 通过浏览器选择查看Web页面尾部区域的代码内容

    ![文件包含示例-1](../image/Part_3/文件包含示例-1.png)

2. 在模板文件的index.html源码文件中，从<body>标签内剪切这部分代码，并写入footer.php文件中

    ![文件包含示例-2](../image/Part_3/文件包含示例-2.png)

    ```php
    # /demo02/common/footer.php文件（未展开）
    <!-- Start of bottom wrapper -->
    <div id="bottom_wrapper"...><!-- End of bottom wrapper -->

    <!-- Start of copyright wrapper -->
    <div id="copyright_wrapper"...><!-- End of copyright wrapper -->

    <div style="display:none"...>
    ```

3. 修改模板文件下的所有HTML文件，统一修改为PHP文件，并统一在移除尾部代码的位置添加include语法。以index.html文件为例，重命名index.html为index.php
    ...
    <body>
        ...
        <div id="header_wrapper"...><!-- End of header wrapper -->
    
        <!-- Start of message wrapper -->
        <div id="message_wrapper"...><!-- End of message wrapper -->
    
        <!-- Start of content wrapper -->
        <div id="contentwrapper"...><!-- End of content wrapper -->
    ...
    [index.html] -> [重构] -> [重命名];使用IDE对模板文件重命名时需要注意勾选“搜索索引”和“在注释和字符中搜索”，勾选两者后在重命名index.html文件时，其它代码文件中有引用index.html文件的部分，全部都会统一自动替换为引用新的index.php文件

    ![文件包含示例-3](../image/Part_3/文件包含示例-3.png)

#### 16.3.3 分离通用文件——头部

尾部通用代码的分离比较简单，所有的尾部通用代码都在\<body\>标签下，直接复制\<div\>标签中的代码内容即可实现通用。头部通用文件的难点在于找到不同Web页面之间的代码差异，头部通用代码就不再仅限于\<body\>标签，\<head\>标签内的代码也需要兼顾处理，因为不同Web页面之间的头部代码都存在差异。

在\<body\>标签下，各个Web页面中最明显的差异在于，首页文件（index.php）单独使用了“滚动条”（slider），而除了首页文件以外，其它Web页面文件中都使用了“面包屑导肮”（breadcrumb_wrapper），因此头部通用代码文件需要针对index.php文件做特殊处理。除此以外，每个页面中的<head>标签下都有一些细微的差异，都需要单独进行处理。

由于头部通用代码同时涉及到\<head\>和\<body\>标签的代码，在HTML中\<head\>标签本身也被称为“头部”，为了便于区分，下文中所有的“头部通用代码”均指\<body\>标签部分的代码、head代码指\<head\>标签部分的代码。

1. 通过浏览器选择查看Web页面头部区域的代码内容

    ![文件包含示例-4](../image/Part_3/文件包含示例-4.png)

2. 从模板的index.php文件中移除这部分代码，并写入header.php文件中

    `/demo02/common/header.php`文件（未展开）
...
1. 从其它代码文件中移除面包屑导航代码，并追加写入header.php文件中。以contact.php文件为例

    `/demo02/common/header.php`文件（未展开）

    ```php
    # header_wrapper是从index.php文件中剪切
    <!-- Start of header wrapper -->
    <div id="header_wrapper"...><!-- End of header wrapper -->

    # breadcrumb_wrapper是从contact.php文件中剪切
    <!-- Start of breadcrumb wrapper -->
    <div class="breadcrumb_wrapper"...><!-- End of breadcrumb wrapper -->
    ```

    `/demo02/contact.php`文件（未展开）
    ...
    此时访问index.php首页，可以看到相比较模板原文件的Web页面，修改后的index.php页面多了一个面包屑导航。

    ![文件包含示例-5](../image/Part_3/文件包含示例-5.png)

    同时，从index.php文件剪切到header.php文件的滚动条代码，在contact等其它页面中已经存在代码了，但从Web页面看上去似乎没有生效，这可能是JS渲染的问题导致的。

    ![文件包含示例-6](../image/Part_3/文件包含示例-6.png)

4. 对比index.php与contact.php文件的head代码，观察两者差异

    查看index.php文件差异（仅展示差异处）
    ...
    <head>
    ...
    <title>Business Essentials</title>    //第32行，每个Web页面的标题不同
    ...
    <script type="text/javascript">       //第66行
        jQuery(window).load(function () {

        jQuery('.slider').flexslider();    //只有index.php文件中存在这一行，这就是其它页面在引用通用代码后，仍未正常显示滚动条的原因
        ...
    </script>
    ...
    </head>
    ...
    <head>
    ...
    <title>Business Essentials</title>    //第31行，每个Web页面的标题不同
    ...
    <script type="text/javascript">       //第65行
        ...
        jQuery('form#contact_form').submit(function () {...});    //contact.php页面独有的表单
        ...
    </script>
    ...
    </head>
...
5. 将index.php文件中的\<head\>标签下的所有代码剪切到head.php文件中
