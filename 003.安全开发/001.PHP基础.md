...
2. 从模板的index.html源码文件中移除这部分代码，并写入footer.php文件中
...
头部与尾部的区别在于，模板代码文件中所有页面的尾部都是相同的，但不同页面的头部之间存在差异。首页文件（index.php）单独使用了“滚动条”（slider），而除了首页文件以外，其它页面文件中都使用了“面包屑导肮”（breadcrumb_wrapper），因此头部通用代码文件需要针对index.php文件做特殊处理
