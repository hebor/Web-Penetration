...
1. 通过AI生成一个HTML登陆页面代码，将代码写入`admin-cookie.php`文件
...
3. 对HTML表单转发的数据进行校验（下文中不再对`admin-cookie.php`文件的HTML代码重复展示）
...
$data=mysqli_query($con, $sql);    //返回SQL查询的结果
if(mysql_num_rows($data) > 0){     //当SQL查询结果的行数不为0则判断账号密码正确
